...
function normalizeAccessModes(value: unknown): AccessMode[] {
  ...
  return value.filter(
    (mode): mode is AccessMode => mode === "read" || mode === "write",
  );
}
...
function normalizeProtectedPathsConfig(input: unknown): ProtectedPathsConfig {
  ...
  for (const [pattern, deniedModes] of Object.entries(
...
function getMatchType(normalizedPattern: string): MatchType {
  if (
    ...
    WINDOWS_ABSOLUTE_PATH.test(normalizedPattern)
  )
    return "absolute";
...
async function loadProtectedPathRules(cwd: string) {
  const globalConfig = await loadConfigFromFile(
    resolve(agentDir, "settings.json"),
  );
  const projectConfig = await loadConfigFromFile(
...
function matchesRule(absolutePath: string, cwd: string, rule: CompiledRule) {
  ...
  if (rule.matchType === "relative") {
    const relativePath = normalizeForMatching(
  ...
  return pathSegments.some((segment) =>
    matchesGlob(segment, rule.normalizedPattern),
  );
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const matchedRule = compiledRules.find(
      (rule) =>
        rule.deniedModes.has(mode) && matchesRule(absolutePath, ctx.cwd, rule),
    );
    if (!matchedRule) {
      return undefined;
    }
    ...
    if (ctx.hasUI) {
      ctx.ui.notify(
        `Blocked ${event.toolName} on protected path: ${absolutePath}`,
        "warning",
      );
    }
